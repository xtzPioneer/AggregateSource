...
final class BaiduTests: XCTestCase {
    ...
    func testExample() throws {
        ...
        let holidays = BDHolidays()
        holidays.getHolidays(year: Date().year) { result in
            switch result {
    ...
    func testCombineExample() throws {
        ...
        holidays.publisherHolidays(Date().year).sink { completion in
            switch completion {
            case .finished:
                print("finished")
            case .failure(let error):
